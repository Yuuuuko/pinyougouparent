 //控制层 
app.controller('goodsController' ,function($scope,goodsService,uploadService,itemCatService,typeTemplateService,$controller,$location){
    ...
    //商品上架
    //审核商品
    $scope.goodsMarketAble=function () {
        goodsService.goodsMarketAble($scope.selectIds).success(function (data) {
            if(data.status){
                $scope.reloadList();//刷新列表
                $scope.selectIds=[];
            }else {
